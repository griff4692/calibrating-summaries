...
class LongT5ForConditionalGeneration(LongT5PreTrainedModel):
    ...
    def __init__(self, config: LongT5Config):
        ...
        if hasattr(config, 'contrastive_classifier') and config.contrastive_classifier:
            self.contrast_projection = nn.Linear(config.d_model, config.d_model, bias=True)
    ...
    def forward(
        ...
        abstract the aim of this article is to summarize the studies have shown that owning a dog
        ```"""
        use_cache = use_cache if use_cache is not None else self.config.use_cache
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
