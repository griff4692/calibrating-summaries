...
# https://github.com/amazon-research/summary-reference-revision/blob/3e8149443d24564ae2a6e710e7eac897aadeb51e/src/comp_med_dsum_eval/preprocess/fragment_utils.py
...
def parse_extractive_fragments(source_toks, target_toks, remove_stop=False):
    if remove_stop:
        source_toks = [t for t in source_toks if t.lower() not in STOPWORDS]
        target_toks = [t for t in target_toks if t.lower() not in STOPWORDS]
...
class Fragments(object):
    ...
    def coverage(self, summary_base=True):
        ...
        denominator = len(self.summary) if summary_base else len(self.reference)
